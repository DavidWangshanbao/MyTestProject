...
def load_resource(name):
    ...
    try:
        if is_ST3():
            ...
            filename = name
            return load_utf8(os.path.join('markdowntohtml',filename))
...
class Compiler(object):
    ...
    def get_title(self):
        ...
        return '<title>%s</title>' % cgi.escape(self.title or "")
    ...
    def run(self, view, wholefile=False, preview=False,contents=None):
...
class MarkdownCompiler(Compiler):
    ...
    def process_extensions(self, extensions):
        ...
        if (
            pygments_css is None
                # and            self.settings.get("enable_highlight") is True
        ):
        ...
        base_path = self.settings.get('builtin').get("basepath") if hasattr(self,'builtin') else None
        if base_path is None:
            base_path = ""
    ...
    def get_config_extensions(self, default_extensions):
        config_extensions = self.settings.get('enabled_extensions') if hasattr(self,'settings') else 'default'
    ...
    def parser_specific_convert(self, markdown_text):
        ...
        config_extensions = self.get_config_extensions( [
    "extra", "toc",
    "meta", "sane_lists", "smarty", "wikilinks",
    "admonition"
])
