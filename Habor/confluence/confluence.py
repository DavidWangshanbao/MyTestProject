...
class Confluence(object):
    ...
    def write_page(self,space, title, content, parent=None):
        parent_id = None
        if not parent is None:
            try:
                # Find out the ID of the parent page
                if self._token2:
                    parent_id = self._server.confluence2.getPage(self._token2, space, parent)['id']
                else:
                    parent_id = self._server.confluence1.getPage(self._token, space, parent)['id']
        ...
        try:
            if self._token2:
                existing_page = self._server.confluence2.getPage(self._token2, space, title)
            else:
                existing_page = self._server.confluence1.getPage(self._token, space, title)
        ...
        if self._token2:
            existing_page = self._server.confluence2.storePage(self._token2, existing_page)
        else:
            existing_page = self._server.confluence1.storePage(self._token, existing_page)
