...
def queryByLevel(session, baseurl, level, body):
    _baseurl = _get_prefix(baseurl)
    url = _baseurl + '_queryByLevel'
    return curl(url, session).headers(headers).params(level=level).body(body).perform()
